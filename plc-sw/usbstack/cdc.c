...
void initCDC(void) {
    ...
    // JTR The function usb_init() is now called from main.c prior to anything else
    // belonging to the CDC CLASS
    // If we have multiple functions we want the USB initialization to be in only one
    // consistant place.

    // usb_init(cdc_device_descriptor, cdc_config_descriptor, 
    //	    cdc_str_descs, USB_NUM_STRINGS);	
    // TODO: Remove magic with macro

    // The sort of things we would do in InitCDC would be to setup I/O pins and the
    // HARDWARE UART so it is not transmitting junk between a RESET and the device
    // being enumerated. Hardware CTS/RTS would also be setup here if being used.
    ...
    // JTR These things below should be done after the device is enumerated and
    // configured.
    ...
    //                                   cdc_acm_in_buffer, NULL, cdc_acm_in);
    //	usb_register_endpoint(2, USB_EP_INOUT, CDC_BUFFER_SIZE, cdc_rx_buffer,
    //	                                 cdc_tx_buffer, cdc_rx, cdc_tx);
    // NO! 	usb_register_sof_handler(cdc_flush_tx);

    // JTR change we are separating the basic stack from the user function so we don't
    // do this here
    //      usb_start();
}
...
void user_configured_init(void)
{
    ...
    // This function belongs to the current USB function and IS NOT generic. This is
    // CLASS specific and will vary from implementation to implementation.
    ...
    //				for (i=1; i<MAX_CHIP_EP; i++) {   // JTR change index from 0 to 1 
    //				                                  // as EP0 already armed
    //	#else
    //				for (i=1; i<16; i++) {
    //	#endif
    //					/* Configure buffer descriptors */
    //	#if USB_PP_BUF_MODE == 0
    //					usb_bdt[USB_CALC_BD(i, USB_DIR_OUT, USB_PP_EVEN)].BDCNT  = 
    //					                                    endpoints[i].buffer_size;
    //					usb_bdt[USB_CALC_BD(i, USB_DIR_OUT, USB_PP_EVEN)].BDSTAT =
    //					                                    UOWN + DTSEN;
    //					usb_bdt[USB_CALC_BD(i, USB_DIR_IN, USB_PP_EVEN)].BDCNT  = 0;	
    //					// Set DTS => First packet inverts, ie. is Data0
    //					usb_bdt[USB_CALC_BD(i, USB_DIR_IN, USB_PP_EVEN)].BDSTAT =
    //					                                    DTS + DTSEN;
    //	#else
    ...
    // JTR change. Handle only IN requirements for the NOTICE INTERRUPT EP. There is no
    // OUT NOTICE INTERRUPT EP
    // JTR discontinued using this.
    //  usb_register_endpoint(1, USB_EP_IN, CDC_NOTICE_BUFFER_SIZE, NULL,
    //                                      cdc_acm_in_buffer, NULL, cdc_acm_in);

    // JTR DEBUG temp disable handlers and replace with NULLs. This is for testing
    // purposes only. as we do not want handlers for the CDC at this time.
    // usb_register_endpoint(2, USB_EP_INOUT, CDC_BUFFER_SIZE, cdc_rx_buffer,
    //                                           cdc_tx_buffer, cdc_rx, cdc_tx);

    // JTR discontinued using this
    //  usb_register_endpoint(2, USB_EP_INOUT, CDC_BUFFER_SIZE, cdc_rx_buffer,
    //                                              cdc_tx_buffer, NULL, NULL);
    ...
    // JTR N/A endpoints[i].buffer_size;
    usb_bdt[USB_CALC_BD(2, USB_DIR_OUT, USB_PP_EVEN)].BDCNT  = CDC_BUFFER_SIZE;
    ...
    // Set DTS => First packet inverts, ie. is Data0
    usb_bdt[USB_CALC_BD(2, USB_DIR_IN, USB_PP_EVEN)].BDSTAT = DTS + DTSEN;
...
void cdc_setup(void){
    ...
    switch (packet[USB_bmRequestType] 
            & (USB_bmRequestType_TypeMask | USB_bmRequestType_RecipientMask)) {
        case (USB_bmRequestType_Class | USB_bmRequestType_Interface):
            switch (packet[USB_bRequest]) {
                ...
                case CDC_GET_LINE_CODING:           // Optional, strongly recomended
                    ...
                    // JTR common addition for STD and CLASS ACK
                    usb_ack_dat1( rbdp, reply_len );

                    //rbdp->BDCNT = reply_len;  // JTR done above
                    //usb_ack(rbdp);	// JTR depreciated

                    // JTR why bother? There is nothing more to do.
...
// JTR handling an OUT token In the CDC stack this is the only function that handles an
// OUT data stage.
void cdc_set_line_coding_data( void ) {
...
bdp->BDADDR[USB_bmRequestType]	= USB_bmRequestType_D2H | USB_bmRequestType_Class 
