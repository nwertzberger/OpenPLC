...
#pragma code
   ...
   void _reset(void)
   {
    ...
    //the bootloader but with programmer, addresses 0x08 and 0x18 would end up programmed with 0xFFFF.
    ...
    //When the output hex file of this project is programmed with the bootloader, these sections do not
    //get bootloaded (as they overlap the bootloader space).  If the output hex file is not
    //programmed using the bootloader, then the below goto instructions do get programmed,
    //and the hex file still works like normal.  The below section is only required to fix this scenario.
