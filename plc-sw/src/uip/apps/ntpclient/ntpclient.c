...
static struct ntpclient_state ntpc_s;
...
ntp_get_adjusted(struct ntp_time *time) {
  ...
  if(!ntpc_s.adjust) return;

  /* calculate time since last ntp update */
  ntp_time_diff(time, &ntpc_s.last_sync, &diff);

  /* and adjust it according to current adjustment factor */
  ntp_time_multiply(&diff, ntpc_s.adjust);
...
PT_THREAD(handle_ntp(void))
{
  ...
  ntpc_s.last_sync.seconds = 0;
  ntpc_s.last_sync.fraction = 0;
  ntpc_s.adjust = 0;
  turn_around = 0xffff;

  /* try_again: */
  ntpc_s.ticks = CLOCK_SECOND;
  ...
  while(1) {
    ...
    send_request();
    timer_set(&ntpc_s.timer, ntpc_s.ticks);
    PT_WAIT_UNTIL(&ntpc_s.pt, uip_newdata() || timer_expired(&ntpc_s.timer));
...
#ifdef NTP_DEBUG
      printf("NTP: received reply after = %lu.%05u sec\n", 
	...
	if(diff.fraction < turn_around) {
#ifdef NTP_DEBUG
	  printf("NTP: shortest turn around time so far\n");
#endif
	  ...
	  ntp_time_shr(&diff);
#ifdef NTP_DEBUG
	  printf("NTP: turn around half diff = %lu.%05u (%u,0x%x)\n", 
		 diff.seconds, 99999*diff.fraction/65535, 
...
#ifdef NTP_DEBUG
	  printf("NTP: adjusted ltime = %lx/%x\n", ltime.seconds, ltime.fraction);
#endif
	}
      }
...
#ifdef NTP_DEBUG
	  printf("NTP: diff = %lu.%05u (%u)\n", diff.seconds, 
		 99999*diff.fraction/65535, diff.fraction);
#endif

	  ntp_time_diff(&ltime, &ntpc_s.last_sync, &diff2);  /* T_HW - T_SYNC */

	  ntpc_s.adjust = ntp_time_divide(&diff, &diff2);		
#ifdef NTP_DEBUG
	  printf("NTP: adjust = %d (%f)\n", s.adjust, s.adjust / 65535.0);
#endif

	  ntpc_s.state = NTP_STATE_SYNCHONIZED;
	}
	...
	// just print time for debugging reasons
	{
	  ...
	  printf("NTP: time: %s %d. %s %d %d:%02d:%02d (%s)\n", 
	...
	ntpc_s.last_sync.seconds = time.seconds;
	ntpc_s.last_sync.fraction = time.fraction;
	...
	if(ntpc_s.state == NTP_STATE_NO_TIME) {
	  /* next request sequence after 30 seconds */
	  ntpc_s.ticks = CLOCK_SECOND * 30; 
	  ntpc_s.state = NTP_STATE_INITIALIZED;
	} else {
	  /* next request sequence after XX seconds */
	  ntpc_s.ticks = CLOCK_SECOND * NTP_REQ_CYCLE; 
	}
	
	timer_set(&ntpc_s.timer, ntpc_s.ticks);
	PT_WAIT_UNTIL(&ntpc_s.pt, timer_expired(&ntpc_s.timer));
      } else {
	/* next in-sequence request after 1/10 second */
	ntpc_s.ticks = CLOCK_SECOND/10; 
	timer_set(&ntpc_s.timer, ntpc_s.ticks);
	PT_WAIT_UNTIL(&ntpc_s.pt, timer_expired(&ntpc_s.timer));
      }
      ...
      if(ntpc_s.ticks < CLOCK_SECOND * 60) {
	ntpc_s.ticks *= 2;
      }
    }
  }
...
ntpclient_init(void)
{
  ...
  ntpc_s.state = NTP_STATE_NO_TIME;

  uip_ipaddr(addr, 192,168,1,10);    /* home server elara */
//  uip_ipaddr(addr, 192,53,103,108);  /* ntp1.ptb.de */
  ntpc_s.conn = uip_udp_new(&addr, HTONS(NTP_PORT));
  if(ntpc_s.conn != NULL) {
...
{
	time->seconds = clock_time();
}


void clock_set_ntptime(struct ntp_time *time)
{
	clock_set_time(time->seconds);
}
